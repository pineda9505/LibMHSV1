...
namespace LibMHSV1
{
    public class F07Retenciones : Declaracion
    {
        IConexion cn;
        ISQLPack sq;
        public List<F07Retenciones> Lista { get; set; }
        ...
        public F07Retenciones(string nit, DateTime fecha, string tipoDoc, string serie, string numDoc, double subTot, double retencion, string dui, byte anexo)
        {
            Nit = nit;
            Fecha = fecha;
            Tipodoc = tipoDoc;
        ...
        public F07Retenciones(byte periodo, int ejercicio)
        {
            cn = new ConexionSql();
            if (cn.Conecto())
            {
                ...
                {
                    sq = new SQLPack();
                    sq.ProcedimientoAlmacenado("sp_F07RetencionesMostrar", cn.getSqlcon());
                    ...
                    {
                        Lista = new List<F07Retenciones>();
                        while (sq.Dr.Read())
                        {
                            Lista.Add(new F07Retenciones(sq.Dr["NIT"].ToString()
                                                         ...
                                                         , Convert.ToDouble(sq.Dr["RETENCION"])
                                                         , sq.Dr["DUI"].ToString()
                ...
                {
                    Registros.Escribir("Libros MH / Retenciones", ex.Message);
                }
                finally
                {
                    cn.Desconectar();
